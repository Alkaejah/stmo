...
const TopFiveByPlaceOfResidency = ({
  showAll = false,
}: {
  showAll?: boolean;
}) => {
  ...
  useEffect(() => {
    if (!isPending && data?.items) {
      ...
      (data.items as HistoricalTicket[]).forEach((ticket) => {
        const municipality = ticket.violatorAddress?.municipality;
        const cityProvince = ticket.violatorAddress?.cityProvince;
        const location =
          municipality && cityProvince
            ? `${municipality}, ${cityProvince}`
...
export default TopFiveByPlaceOfResidency;
