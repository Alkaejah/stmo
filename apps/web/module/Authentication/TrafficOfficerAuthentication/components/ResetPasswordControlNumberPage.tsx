...
import useBackOfficerForgotPassword from "@/common/hooks/BackOffice/useBackOfficerForgotPassword";
...
const OfficerResetPasswordControlNumber = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [controlNumber, setControlNumber] = useState<string>("");
  const router = useRouter();
  const { mutateAsync: checkOfficerControlNumber } =
    useBackOfficerForgotPassword();
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  const onSubmit = async (formData: IResetPassword) => {
    try {
      ...
      const response = await checkOfficerControlNumber(modifiedFormData);
      ...
      if (!response?.error) {
        ...
        if (typeof window !== "undefined") {
          localStorage.setItem(
            "controlNumber",
            formData.backOfficerControlNumber.trim(),
          );
        }
        toast.success("Control number verified. Redirecting...");
        setTimeout(() => {
          router.push(
            `/stmo/officers/reset-password-control-number/reset-password`,
          );
        }, 1000);
      } else {
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <Input
                        ...
                        type="text"
                        className="w-full"
                        placeholder="ECN-XXXXXXXXXXXX"
                        {...register("backOfficerControlNumber", {
                          required: "Officer control number is required!",
                          onChange: (e) =>
                            setControlNumber(e.target.value.trim()), // Update controlNumber directly
                        })}
                      />
                      {errors.backOfficerControlNumber && (
                        <p className="text-red-500 text-sm mt-1">
                          {errors.backOfficerControlNumber.message}
                        </p>
                      )}
                    </div>
                  </div>
...
export default OfficerResetPasswordControlNumber;
