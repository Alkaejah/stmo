"use client";
import { Button } from "@/common/components/shadcn/ui/button";
import { Input } from "@/common/components/shadcn/ui/input";
import { Typography } from "@/common/components/ui/Typography";
import { WidthWrapper } from "@/common/components/WidthWrapper";
import { LucideArrowLeft, LucideRefreshCw, LucideUpload } from "lucide-react";
import React, { useEffect, useState } from "react";
import { useForm } from "react-hook-form";
import toast from "react-hot-toast";
import { FileWithPath, useDropzone } from "react-dropzone";
import NextImage from "next/image";
import useGetDriverInfo from "@/common/hooks/Drivers/useGetDriverInfo";
import useDriverSessionStore from "@/common/store/useDriverSessionStore";
import { Spinner } from "@/common/components/ui/Spinner";
import useAddDriverProfilePicture from "@/common/hooks/Drivers/useAddDriverProfilePicture";
import usePhotoStore from "@/common/store/usePhotoStore";
import useUpdateDriverInfo from "@/common/hooks/Drivers/useUpdateDriverInfo";
import useDeleteDriverProfilePicture from "@/common/hooks/Drivers/useDeleteDriverProfilePicture";
import Link from "next/link";

interface IDriver {
  firstName: string;
  lastName: string;
  username: string;
}

const Index = () => {
  const session = useDriverSessionStore((state) => state);
  const driverId = String(session.id || "");
  const { data, isPending, refetch } = useGetDriverInfo(driverId as string);
  const [isLoading, setIsLoading] = useState(false);
  const photoId = data?.item?.profilePicture?.[0]?._id;
  const { mutateAsync: uploadProfilePicture } =
    useAddDriverProfilePicture(driverId);
  const { mutateAsync: udpateDriverInfo } = useUpdateDriverInfo(driverId);
  const { mutateAsync: removeProfilePicture } =
    useDeleteDriverProfilePicture(photoId);
  const photos = usePhotoStore((state) => state.photos);
  const setPhotos = usePhotoStore((state) => state.setPhotos);
  const [photo, setPhoto] = useState<FileWithPath | null>(null);
  const [photoPreview, setPhotoPreview] = useState<string | null>(null);

  const {
    register,
    handleSubmit,
    setValue,
    formState: { errors },
  } = useForm<IDriver>();

  const removePhoto = () => {
    setPhoto(null);
    setPhotoPreview(null);
    toast.success("Photo removed!");
  };

  const handleRemoveProfilePhoto = async () => {
    try {
      const photoId = data?.item?.profilePicture?.[0]?._id;

      if (!photoId) {
        toast.error("No profile picture to remove!");
        return;
      }

      await removeProfilePicture({
        _id: photoId,
        description: "",
        tags: "",
      });

      toast.success("Profile picture removed successfully!");
      setPhotoPreview(null);
      await refetch(); // Refetch data after removing the photo
    } catch (err) {
      toast.error("Failed to remove profile picture.");
      console.error("Error removing profile picture:", err);
    }
  };

  const updatePhotosInDb = async () => {
    const toAddPhotos = photos
      .filter((photo) => !photo._id)
      .map(async (photo) => await uploadProfilePicture(photo));
    try {
      const items = await Promise.all(toAddPhotos);
      items.forEach((item) => {
        const message = String(item.message);
        toast.success(message, { id: message });
      });
    } catch (err) {
      toast.error(String(err));
    }
  };
  const handleSave = async () => {
    await updatePhotosInDb();
  };

  const onSubmit = async (formData: IDriver) => {
    try {
      setIsLoading(true);
      const modifiedFormData = {
        ...formData,
        firstName: formData.firstName,
        lastName: formData.lastName,
        username: formData.username,
      };
      const response = await udpateDriverInfo(modifiedFormData);
      if (!response.error) {
        toast.success("Information updated successfully!");
      } else {
        toast.error("Failed to update information.");
      }
      if (photos.length > 0) {
        await handleSave();
      }
    } catch (err) {
      console.error("Error:", err);
      toast.error("Something went wrong. Please try again.");
    } finally {
      setIsLoading(false);
    }
  };

  const { getRootProps, getInputProps } = useDropzone({
    accept: { "image/jpeg": [], "image/png": [], "image/jpg": [] },
    maxFiles: 1,
    onDrop: (acceptedFiles) => {
      if (photo) {
        toast.error("Remove uploaded photo first!");
        return;
      }

      const file = acceptedFiles[0];
      if (!file) {
        toast.error("Invalid file. Please upload a valid image.");
        return;
      }

      const img = new Image();
      img.src = URL.createObjectURL(file);
      img.onload = () => {
        const newPhoto = {
          file,
          description: "",
          tags: "",
          isMain: photos.length === 0,
        };
        setPhotos([...photos, newPhoto]);
        setPhoto(file);
        setPhotoPreview(URL.createObjectURL(file));
        toast.success("Photo uploaded successfully!");
      };

      img.onerror = () => {
        toast.error("Error loading image. Please try again.");
      };
    },
    onDropRejected: () => {
      toast.error("Only PNG, JPEG, or JPG files are allowed.");
    },
    noClick: !!data?.item?.profilePicture?.length, // Disable click if profile picture exists
    noKeyboard: !!data?.item?.profilePicture?.length, // Disable keyboard interaction if profile picture exists
  });

  useEffect(() => {
    if (!isPending && data?.item) {
      setValue("firstName", data?.item?.firstName || "N/A");
      setValue("lastName", data?.item?.lastName || "N/A");
      setValue("username", data?.item?.username || "N/A");

      if (data?.item?.profilePicture?.[0]?.key) {
        setPhotoPreview(`/assets/${data.item.profilePicture[0].key}`);
      }
    }
  }, [data, isPending, setValue]);

  return (
    <WidthWrapper width="full">
      <div className="relative w-full min-h-screen flex justify-center items-center bg-cover bg-center bg-[url('/Aerial_Shot.png')] px-4 sm:px-6 md:px-8 lg:px-8 md:py-8 lg:py-8">
        <div className="relative z-10 w-full max-w-full h-[90vh] bg-primary bg-opacity-50 shadow-2xl rounded-xl flex flex-col justify-center items-center p-6 sm:p-10 md:p-12 lg:p-14">
          <div className="flex justify-center text-center mb-4">
            {isPending ? (
              <Spinner size="md">Loading...</Spinner>
            ) : (
              <div className="bg-white rounded-lg shadow-md p-4 sm:p-10 md:p-10 lg:p-10 w-full max-w-6xl mt-4">
                <Typography
                  variant="h2"
                  fontWeight="bold"
                  className="text-center mb-4 text-lg sm:text-lg md:text-xl lg:text-2xl px-4"
                >
                  Update Personal Information
                </Typography>

                <form onSubmit={handleSubmit(onSubmit)} className="w-full">
                  <div className="relative flex flex-col items-center">
                    <div
                      {...getRootProps()}
                      className={`relative w-32 h-32 sm:w-24 sm:h-24 md:w-32 md:h-32 lg:w-36 lg:h-36 overflow-hidden bg-gray-100 ${
                        photo || data?.item?.profilePicture?.length
                          ? "cursor-default border-4 border-red-500"
                          : "hover:bg-gray-200 border-2 border-dashed cursor-pointer"
                      } rounded-full flex items-center justify-center transition-all duration-300`}
                    >
                      {photoPreview ? (
                        <div className="relative w-full h-full">
                          <NextImage
                            src={photoPreview}
                            alt="Preview"
                            layout="fill"
                            objectFit="cover"
                            className="rounded-full"
                          />
                        </div>
                      ) : (
                        <div className="flex flex-col items-center text-gray-400">
                          <LucideUpload size={24} />
                          <span className="text-xs mt-2 text-center">
                            Click here or drop photo
                          </span>
                        </div>
                      )}
                      <input
                        {...getInputProps()}
                        disabled={
                          !!photo || !!data?.item?.profilePicture?.length
                        }
                      />
                    </div>

                    {data?.item?.profilePicture?.length > 0 && !photo && (
                      <button
                        type="button"
                        onClick={handleRemoveProfilePhoto}
                        className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-red-500 text-white px-4 py-1.5 rounded-full shadow-lg hover:bg-red-600 transition text-sm font-semibold z-10 border-4 border-red-500"
                      >
                        Remove
                      </button>
                    )}
                    {photo && (
                      <button
                        type="button"
                        onClick={removePhoto}
                        className="absolute -bottom-4 left-1/2 transform -translate-x-1/2 bg-red-500 text-white px-4 py-1.5 rounded-full shadow-lg hover:bg-red-600 transition text-sm font-semibold z-10 border-4 border-red-500"
                      >
                        Remove
                      </button>
                    )}
                  </div>

                  <div className="lg:space-y-4 mt-4">
                    <div className="flex flex-col sm:items-start items-center space-y-2 w-full">
                      <label
                        htmlFor="firstName"
                        className="block text-gray-700 font-bold text-center sm:text-left w-full"
                      >
                        First Name
                      </label>
                      <Input
                        id="firstName"
                        {...register("firstName")}
                        placeholder="Enter First Name"
                        className="w-full"
                      />
                    </div>

                    <div className="flex flex-col sm:items-start items-center space-y-2 w-full">
                      <label
                        htmlFor="lastName"
                        className="block text-gray-700 font-bold text-center sm:text-left w-full"
                      >
                        Last Name
                      </label>
                      <Input
                        id="lastName"
                        {...register("lastName")}
                        placeholder="Enter Last Name"
                        className="w-full"
                      />
                    </div>

                    <div className="flex flex-col sm:items-start items-center space-y-2 w-full">
                      <label
                        htmlFor="username"
                        className="block text-gray-700 font-bold text-center sm:text-left w-full"
                      >
                        Username
                      </label>
                      <Input
                        id="username"
                        {...register("username")}
                        placeholder="Enter Username"
                        className="w-full"
                      />
                    </div>
                  </div>

                  <div className="mt-6 flex flex-col sm:flex-row sm:justify-between gap-4">
                    <Button
                      type="submit"
                      size="lg"
                      className="w-full md:w-32 sm:w-auto bg-secondary hover:bg-blue-400 text-white font-bold py-2 px-4 rounded flex items-center justify-center gap-2"
                      disabled={isLoading}
                    >
                      {isLoading ? (
                        <Spinner size="sm" />
                      ) : (
                        <LucideRefreshCw size={18} />
                      )}{" "}
                      Update
                    </Button>
                    <Link href={`/driver`} className="w-full sm:w-auto">
                      <Button
                        type="button"
                        size="lg"
                        className="w-full md:w-32 sm:w-auto bg-gray-500 hover:bg-gray-600 text-white font-bold py-2 px-4 rounded flex items-center justify-center gap-2"
                      >
                        <LucideArrowLeft size={18} /> Back
                      </Button>
                    </Link>
                  </div>
                </form>
              </div>
            )}
          </div>
        </div>
      </div>
    </WidthWrapper>
  );
};

export default Index;
