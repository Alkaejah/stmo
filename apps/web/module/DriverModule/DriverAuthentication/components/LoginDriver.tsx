...
import useLogin from "@/common/hooks/Drivers/useLogin";
...
const LoginDriver = () => {
  ...
  const { mutateAsync: loginDriver, isPending: isLoginPending } = useLogin();
  ...
  const onSubmit = async (formData: T_Drivers) => {
    if (rememberMe) {
      ...
      onSuccess: (data: any) => {
        if (!data.error && !isLoginPending) {
          if (data.action && data.action.link) {
            queryClient.invalidateQueries({ queryKey: ["session-driver"] });
      ...
      <div className="relative z-10 w-full max-w-full min-h-[90vh] bg-primary bg-opacity-50 shadow-2xl rounded-xl flex flex-col justify-center items-center p-6 sm:p-10 md:p-12 lg:p-14">
        <div className="flex justify-center text-center mb-4">
          <Image
            ...
            width={800}
            height={800}
            className="object-fit w-fit h-20 sm:h-24 md:h-28"
          />
        </div>
        <div className="bg-white rounded-lg shadow-md p-6 sm:p-8 md:p-10 w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-lg mx-auto">
          <form onSubmit={handleSubmit(onSubmit)} className="w-full">
            ...
            <div className="flex justify-between items-center mt-4">
              ...
              <Link href={`/drivers-auth/reset-password-control-number`}>
                <Typography
                  fontWeight="semiBold"
